    ...
    for filename in os.listdir(input_img_dir):
        ...
        with rasterio.open(img_path) as src_img:
            with rasterio.open(label_path) as src_label:
                ...
                pad = crop_size // 2   # 四周边缘额外填充的像素数
                pad_top = pad
                pad_bottom = pad_height + pad
                pad_left = pad
                pad_right = pad_width + pad

                # 对图像进行填充
                img_data = src_img.read()
                img_data = np.pad(img_data, ((0, 0), (pad_top, pad_bottom), (pad_left, pad_right)), mode='constant', constant_values=pad_value)
                #img_data = np.pad(img_data, ((0, 0), (0, pad_height), (0, pad_width)), mode='constant', #constant_values=0)

                # 对标签进行填充
                label_data = src_label.read()
                label_data = np.pad(label_data, ((0, 0), (pad_top, pad_bottom), (pad_left, pad_right)), mode='constant', constant_values=pad_value)
                ...
                for y in range(0, height - crop_size + 1, stride):
                    for x in range(0, width - crop_size + 1, stride):
                        ...
                        tf_crop = rasterio.Affine(
                            tf.a,  # 像素宽度保持不变
                            tf.b,  # 旋转参数不变
                            tf.c + (x - pad_left) * tf.a,  # 左上角X坐标加上步长乘以像素宽度（像素宽度保持不变）
                            tf.d,  # 旋转参数不变
                            tf.e,  # 像素高度保持不变   
                            tf.f + (y - pad_top) * tf.e,  # 左上角Y坐标减去步长乘以像素高度（像素高度保持不变） 
                        )
